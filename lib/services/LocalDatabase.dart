...
class LocalDatabase {
  ...
  Future<Database> init() async {
    return await openDatabase(
      join(await getDatabasesPath(), 'collections.db'),
  ...
  Future<Collection> getCollectionByName(String name) async {
    final Database db = await database;
    List<Map<String, dynamic>> maps = await db.query('collections', where: 'name = ?', whereArgs: [name]);
    if (maps.isNotEmpty) {
  ...
  Future<List<String>> getCollectionNames() async {
    ...
    List<Map<String, dynamic>> maps = await db.query('collections', columns: ['name']);
